...
import { apiClientFetch, apiServerFetch } from '../../../../../package/api/api-fetch';

const User1 = '/assets/images/users/user-round.png';
...
const ProfileSection = ({ profileNavigation, user }: ProfileSectionProps) => {
  ...
  const handleLogout = async () => {
    try {
      const data = await apiClientFetch('logout', 'user', {});
  ...
  const handleListItemClick = (event: React.MouseEvent<HTMLDivElement>, index: number, route: string = '') => {
    ...
    handleClose(event);
    router.push(profileNavigation as string);
  };
  const handleToggle = () => {
  ...
  return (
    <>
      <Chip
        ...
        label={<IconChevronDown stroke={1.5} size="24px" />}
        variant="outlined"
        ref={anchorRef}
        aria-controls={open ? 'menu-list-grow' : undefined}
        aria-haspopup="true"
