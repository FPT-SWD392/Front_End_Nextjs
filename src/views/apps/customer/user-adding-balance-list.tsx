...
const UserAddingBalanceList = ({ transactionHistory }: { transactionHistory: TransactionHistory[] }) => {
              ...
              return (
                <TableRow hover tabIndex={-1} key={index}>
                  <TableCell component="th" scope="row" sx={{ cursor: 'pointer' }}>
                    <Typography variant="subtitle1">#{row.transactionId} </Typography>
                  </TableCell>

                  <TableCell>{"VN Pay"}</TableCell>
                  <TableCell align="left">{formatNumber(row.amount)}đ</TableCell>
                  <TableCell align="left">
                    {row.isSuccess ? (
                      <Chip label="Thành công" size="small" chipcolor="success" />
                    ) : (
                      <Chip label="Thất bại" size="small" chipcolor="error" />
                    )}
                  </TableCell>
                  <TableCell align="left">{formatDate(row.transactionDate, "dd/MM/yyyy")}</TableCell>
                </TableRow>
              );
            })}
            {emptyRows > 0 && (
...
export default UserAddingBalanceList;
